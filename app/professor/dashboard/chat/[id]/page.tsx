...
import { useChatTicket, useSendMessage, useCloseTicket, useMarkMessagesAsRead } from "@/hooks/use-chat-tickets"
...
export default function ProfessorChatTicketPage() {
  const params = useParams()
  const router = useRouter()
  const ticketId = params.id as string
  const { ticket, loading, error } = useChatTicket(ticketId)
  const { essays } = useEssays({})
  const { user } = useAuth()
  const { sendMessage, loading: sendingMessage } = useSendMessage()
  const { closeTicket, loading: closingTicket } = useCloseTicket()
  const { markAsRead } = useMarkMessagesAsRead()
  const { uploadProgress, validateFile, MAX_FILE_SIZE, MAX_FILES_PER_MESSAGE } = useChatAttachments()
  const log = useLogger('ChatTicketProfessor', '/professor/dashboard/chat/[id]')
  ...
  useEffect(() => {
    ...
    log.info('Página de chat ticket do professor carregada', {
      metadata: { theme: 'light_forced', ticketId }
    })
  }, [ticketId]) // Removido 'log' das dependências
    ...
    if (ticket?.userId) {
      const fetchStudentName = async () => {
        try {
          ...
          if (userSnap.exists()) {
            setStudentName(userSnap.data().name)
          }
        } catch (error) {
  ...
  // Marcar mensagens como lidas (DESABILITADO TEMPORARIAMENTE PARA DEBUG)
  // useEffect(() => {
  //   if (ticket && user) {
  //     markAsRead(ticket.id)
  //   }
  // }, [ticket, user, markAsRead])
  ...
  const handleCloseTicket = async () => {
    try {
      ...
      router.push('/professor/dashboard/chat')
    } catch (error) {
      toast.error("Erro ao fechar chat. Tente novamente.")
  ...
  if (error || !ticket) {
    return (
      <div className="text-center py-8">
        ...
        <Button onClick={() => router.push('/professor/dashboard/chat')} className="mt-4">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Button 
                variant="ghost" 
                size="sm" 
                onClick={() => router.push('/professor/dashboard/chat')}
              >
              ...
              <div>
                ...
                <div className="flex items-center gap-4 text-sm text-gray-600">
                  <p>{getEssayTitle(ticket.essayId)}</p>
                  <span className="flex items-center gap-1">
                    <User className="h-3 w-3" />
                    {studentName || `Aluno ID: ${ticket.userId}`}
                  </span>
                </div>
              </div>
            </div>
